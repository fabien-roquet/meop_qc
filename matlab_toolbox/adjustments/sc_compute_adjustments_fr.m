...
for itag=1:length(info_deployment.list_tag)
    name_prof = sprintf('%s%s',info_deployment.dir,info_deployment.list_tag(itag).name);
    name_prof = strrep( name_prof, '_lr0', '_fr1');
...
for itag=1:length(info_deployment.list_tag)
      ...
      %  conf_adjustement.list_tag=1;
        ...
        conf_adjustement.nomfig=sprintf('%s%s/%s/calibration_%02d_0_fr1.png',...
        ...
        for jj=1:length(J)-1
            ...
            conf_adjustement.nomfig=sprintf('%s%s/%s/calibration_%02d_%03d_part%02d_fr1.png',...
